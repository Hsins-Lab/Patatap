...
function onKeyDown(event) {
  if(keyData[event.key]) {
    // Select a max point with current max width and height
    var maxPoint = new Point(view.size.width, view.size.height);
    // Create random point using .random() method
    var randomPoint = Point.random();
    // Create a point anywhere in the view pane of screen
    var point = maxPoint * randomPoint;
    // Render this point on current view pane
...
function onFrame(event){
  for (var i = 0; i < circles.length; i++) {
    ...
    if (circles[i].area < 1) {
      ...
      circles.splice(i, 1);   // Remove the cirlce from array
      console.log(circles);
    }
  }
}